            ...
            new ActionListener() {
                ...
                public void actionPerformed(ActionEvent event) {
                    try {
                        ...
                        String sreSqlRemoverId = "delete from `db_senac`.`tbl_senac` wher `id` = " + String.valueOf(cbxId.getSelectedItem()) + ";";
                        Statement stmSqlRemoverId = conexao.createStatement();
                        stmSqlRemoverId.addBatch(sreSqlRemoverId);
                    ...
                    try {
                        int currentIndex = cbxId.getSelectedIndex();
                        if (currentIndex < cbxId.getItemCount() - 1) {
                            cbxId.setSelectedIndex(currentIndex + 1);
                            cbxId.removeItemAt(currentIndex);
                        }
                    } catch (Exception e) {
                        e.printStackTrace();
                    }
        ...
        appTelaDeRemover.getRootPane().addComponentListener(
            new ComponentAdapter() {
                public void componentResized(ComponentEvent event) {
                    ...
                    notificarUsuario(String.format("Largura: %d, Altura: %d", larguraTela, alturaTela));
                }
            }
        );
    }
